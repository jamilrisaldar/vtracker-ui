...
import { googleClientId, isBackendAuthEnabled } from '../config'
    ...
    <div className="flex min-h-svh flex-col items-center justify-center bg-gradient-to-b from-slate-900 via-slate-800 to-teal-950 px-4 py-12">
      <div className="w-full max-w-md rounded-2xl border border-white/10 bg-white/95 p-8 shadow-xl backdrop-blur">
        <div className="mb-8 text-center">
          ...
          <p className="mt-2 text-sm text-slate-600">
            {backend
              ...
              : 'Sign in with your Google account, or use simulated sign-in for local mock data.'}
          </p>
        </div>
            ...
            <>
              {googleClientId ? (
                ...
                <p className="text-center text-sm text-slate-600">
                  No{' '}
                  <code className="rounded bg-slate-100 px-1">VITE_GOOGLE_CLIENT_ID</code>{' '}
                  configured. Use simulated sign-in below, or add your OAuth client ID
                  to <code className="rounded bg-slate-100 px-1">.env</code>.
                </p>
              )}
        ...
        <p className="mt-8 text-center text-xs text-slate-500">
          {backend
            ? 'Session cookie is set by the API (see Swagger: Auth). Project data may still use the local mock store until those endpoints are wired.'
            : 'Mock mode stores data in your browser until the backend is connected.'}
        </p>
      </div>
    </div>
  )
